...
final class CompositionalLayoutManager {
    ...
    private func createHorizontalSection(isSlider: Bool) -> NSCollectionLayoutSection {
        ...
        if (isSlider) {
            ...
            groupSize = NSCollectionLayoutSize(widthDimension: .estimated(100), heightDimension: .estimated(120))
            itemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .fractionalHeight(1.0))
        }

        let item = NSCollectionLayoutItem(layoutSize: itemSize!)
        item.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 0, bottom: 20, trailing: 0) // Öğeler arasındaki boşluk

        let group = NSCollectionLayoutGroup.vertical(layoutSize: groupSize!, subitems: [item])

        let section = NSCollectionLayoutSection(group: group)
        section.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 20, bottom: 0, trailing: 20) // Section içindeki boşluk
